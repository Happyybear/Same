...
-(NSString*) createMd5Sign:(NSMutableDictionary*)dict
{
    ...
    //添加key字段
    [contentString appendFormat:@"key=%@", @"Aa2c021d616d2f7fcf6dee8335c7f362"];
    //得到MD5 sign签名
...
-(NSMutableDictionary *)sendPayWithData:(orderModel*)order
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = @"SEM能效管理-费用缴纳";
    //订单金额,单位（分）
    NSString *order_price   = [NSString stringWithFormat:@"%.0f",[order.fee doubleValue]*100] ;//1分钱测试
    ...
    [packageParams setObject: order.deviceName        forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: order_name        forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject: NOTIFY_URL        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: order.orderID           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"192.168.11.161"    forKey:@"spbill_create_ip"];//发器支付的机器ip
