                     ...
                     andMainView:(UIViewController *)mainVC
{
    if(self = [super init]){
        ...
        mview.frame = self.leftVC.view.bounds;
        mview.backgroundColor = [UIColor blackColor];
        mview.alpha = 0;
        self.mainContentView = mview;
        [self.mainVC.view addSubview:mview];
        //获取左侧tableview
        ...
        self.leftTableview.frame = CGRectMake(0, 0, kScreenWidth - kMainPageDistance, kScreenHeight-100);
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        self.leftTableview.center = CGPointMake(leftTabCenterX, kScreenHeight * 0.5*(1+leftScale)*((SCREEN_H/2 - 50)/(SCREEN_H/2)));
        self.leftTableview.transform = CGAffineTransformScale(CGAffineTransformIdentity, leftScale,leftScale);
        DLog(@"%f",leftScale);
        offx = kScreenHeight * 2/3;
...
-(void)handeTap:(UITapGestureRecognizer *)tap{
    ...
    if ((!self.closed) && (tap.state == UIGestureRecognizerStateEnded))
    {
        ...
        self.leftTableview.center = CGPointMake(kLeftCenterX, kScreenHeight * 0.5*1.7);
...
- (void)closeLeftView
{
    ...
    self.leftTableview.center = CGPointMake(kLeftCenterX, offx);
    self.leftTableview.transform = CGAffineTransformScale(CGAffineTransformIdentity,kLeftScale,kLeftScale);
    self.contentView.alpha = kLeftAlpha;
...
- (void)openLeftView;
{
    ...
    self.leftTableview.center = CGPointMake((kScreenWidth - kMainPageDistance) * 0.5, off);
